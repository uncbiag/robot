...
class PointConvSceneFlowPWC2_6(nn.Module):
    def __init__(
        self,
        input_channel=3,
        ...
        control_shift_factor=0.0,
    ):
        super(PointConvSceneFlowPWC2_6, self).__init__()
        flow_nei = 32
        feat_nei = 16
        sbf = partial(shrink_by_factor, factor=param_shrink_factor)
        self.predict_at_low_resl = predict_at_low_resl
        self.scale = 1
        self.control_shift_factor = control_shift_factor
        # l0: 8192
        ...
        self.flow0 = SceneFlowEstimatorPointConv3(
            sbf(32 + 64),
            sbf(32),
        ...
        self.flow1 = SceneFlowEstimatorPointConv3(
            sbf(64 + 64),
            sbf(64),
            channels=sbf([128, 128]),
            mlp=sbf([128, 64]),
        ...
        self.flow2 = SceneFlowEstimatorPointConv3(
            sbf(128 + 64),
            sbf(128),
            channels=sbf([128, 128]),
            mlp=sbf([128, 64]),
        ...
        self.flow3 = SceneFlowEstimatorPointConv3(
            sbf(256),
            sbf(256),
            flow_ch=0,
            channels=sbf([128, 128]),
    ...
    def forward(self, xyz1, xyz2, color1, color2):
        ...
        feat3, flow3, fea_flow3, shift_flow3 = self.flow3(pc1_l3, feat1_l3, cost3)
        ...
        up_shiftflow2 = self.upsample(
            pc1_l2, pc1_l3, shift_flow3, resol_factor=scaler(5)
        )
        ...
        feat2, resid_flow2, resid_fea_flow2, resid_shift_flow2 = self.flow2(
            pc1_l2, new_feat1_l2, cost2, up_flow2
        )
        flow2 = up_flow2 + resid_flow2
        fea_flow2 = up_feaflow2 + resid_fea_flow2
        shift_flow2 = up_shiftflow2 + resid_shift_flow2
        ...
        up_shiftflow1 = self.upsample(
            pc1_l1, pc1_l2, shift_flow2, resol_factor=scaler(4)
        )
        ...
        feat1, resid_flow1, resid_fea_flow1, resid_shift_flow1 = self.flow1(
            pc1_l1, new_feat1_l1, cost1, up_flow1
        )
        flow1 = up_flow1 + resid_flow1
        fea_flow1 = up_feaflow1 + resid_fea_flow1
        shift_flow1 = up_shiftflow1 + resid_shift_flow1
        ...
        if not self.predict_at_low_resl:
            ...
            up_shift_flow0 = self.upsample(
                pc1_l0, pc1_l1, shift_flow1, resol_factor=scaler(3)
            )
            ...
            _, resid_flow0, resid_fea_flow0, resid_shift_flow0 = self.flow0(
                pc1_l0, new_feat1_l0, cost0, up_flow0
            )
            flow0 = up_flow0 + resid_flow0
            fea_flow0 = up_fea_flow0 + resid_fea_flow0
            shift_flow0 = up_shift_flow0 + resid_shift_flow0
            floweds = [
                flow0 + pc1_l0.detach(),
                flow1 + pc1_l1.detach(),
                flow2 + pc1_l2.detach(),
            ...
            additional_param = {
                "control_points": (pc1_l0 + shift_flow0 * self.control_shift_factor)
                .transpose(2, 1)
                .contiguous(),
                ...
                "control_shift": (shift_flow0 * self.control_shift_factor)
                .transpose(2, 1)
                .contiguous(),
            }
            ...
            fps_pc1_idxs = [fps_pc1_l1, fps_pc1_l2, fps_pc1_l3]
            additional_param = {
                "control_points": (pc1_l1 + shift_flow1 * self.control_shift_factor)
                .transpose(2, 1)
                .contiguous(),
                "control_points_idx": fps_pc1_l1,
                "predict_at_low_resl": self.predict_at_low_resl,
                "control_shift": (shift_flow1 * self.control_shift_factor)
                .transpose(2, 1)
                .contiguous(),
            }
            additional_param.update({"floweds": floweds, "fps_pc1_idxs": fps_pc1_idxs})
